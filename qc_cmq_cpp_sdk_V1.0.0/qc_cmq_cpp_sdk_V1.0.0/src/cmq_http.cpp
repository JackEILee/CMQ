...
void CMQHttp::request(const string &method, const string &url, const string &req, string &rsp)
{
	if(!this->isKeepAlive)
	{
	...
	if(ret != CURLE_OK)
	{
		string error = "Curl Send Request Fail, code:" + CMQTool::int2str(ret) + " ,error:" + curl_easy_strerror(ret);
